...
Version: 2026-04-26 v16
Status: populated with current-state evidence and re-aligned to package-v2 Decision 15 baseline; billing gate resolved by self-hosted runner cutover; plan-gate resolved 2026-04-26 by public-flip; Track A finalized with Owner-deferred rotation; **`main` branch protection applied 2026-04-26 via PR #3 γ gate end-to-end** (Codex propose + distinct Claude Code reviewer verdict pass + 강은구 PR-comment approval + Codex apply) — KPI K4 satisfied by this very action and KPI K2 advances from policy-only to technically enforced. Remaining items in §10 / §16 are non-hosting
Patch record:
...
- v16 records `main` branch protection applied on `hakhamsolution/ax-audit` on 2026-04-26 as the WB2 first deliverable, executed end-to-end as a γ gate `change_guard_rule` cycle (Codex propose via PR #3 + distinct Claude Code reviewer verdict pass + 강은구 PR-comment approval + Codex apply via `gh api -X PUT`); this is the first real γ gate completed in this cycle, satisfying KPI K4 ("γ gate workflow verified end-to-end") and advancing KPI K2 ("Decision 15 adoption") from policy-only to technically enforced on `main`
...
| GitHub branch-protection / rulesets configuration proof | applied 2026-04-26 as the WB2 first deliverable (PR #3 γ gate end-to-end). `gh api repos/hakhamsolution/ax-audit/branches/main/protection` now confirms: `required_status_checks.contexts: ["validate"]`, `required_status_checks.strict: true`, `required_pull_request_reviews.required_approving_review_count: 1`, `required_pull_request_reviews.dismiss_stale_reviews: true`, `enforce_admins.enabled: true`, `allow_force_pushes.enabled: false`, `allow_deletions.enabled: false`. | GitHub repository settings | none (resolved 2026-04-26) | resolved — protection captured in PR #4 body and §14 row below |
...
|---|---|---|---|---|---|
| promote automation | 강은구 | GitHub PR + written approval; CI execution on self-hosted runner; `main` branch protection applied 2026-04-26 (PR #3 γ gate end-to-end) so PR-only is now technically enforced — direct push to `main` is blocked and merges require an approving PR review plus the `validate` status check | yes | AI | 강은구 |
...
| change guard rule | 강은구 | enforced via `main` branch protection (applied 2026-04-26 per PR #3, γ gate `change_guard_rule` end-to-end): require pull_request review (≥1, dismiss stale), required status check `validate`, enforce on admins, no force-push, no deletions. First γ gate cycle completed end-to-end on the very action that turned this enforcement on. | yes | AI | 강은구 |
...
| `hakham-dex[bot]` (GitHub App, App ID 3502063, installation 127042778 on `hakhamsolution/ax-audit`) | GitHub App identity used by Codex/Claude proposer lane to push and open PRs | git push (Contents: write), pull requests (Read & write), metadata (Read) — scoped to `hakhamsolution/ax-audit` only | App private key stored locally outside the repo (`~/Downloads/...private-key.pem`); installation tokens are short-lived (1 hour) | repos and PRs in this single repo only | proposer (GitHub identity layer) | no — App author cannot self-approve; `hakhamsolution` (Owner) is a distinct GitHub actor and supplies the approving review | keep scope narrow to ax-audit; private key custody must move into SOPS+age during the secrets migration |
...
3. `main` branch protection on the now-public `hakhamsolution/ax-audit` is APPLIED 2026-04-26 (PR #3 γ gate end-to-end). Follow-up architectural correction made the same day: AI propose lane was migrated from the human `hakhamsolution` GitHub identity to the `hakham-dex[bot]` App identity (§15.5) so that 강은구 (`hakhamsolution`) can serve as the approver under GitHub's hardcoded self-approval block — the technical enforcement now matches the policy intent of Decision 15 at the GitHub layer too.
...
As of 2026-04-26: the GitHub Actions billing gate is resolved by self-hosted runner cutover; the GitHub plan-gate is resolved by Owner-approved public-flip of `hakhamsolution/ax-audit`; Track A is finalized with a confirmed Slack bot-token exposure in `똘똘이mk2` history that the Owner has explicitly elected NOT to rotate (Owner-accepted residual risk); **`main` branch protection is applied — the WB2 first deliverable was executed end-to-end as a real γ gate `change_guard_rule` cycle, satisfying KPI K4 by the very act of turning protection on and advancing KPI K2 from policy-only to technically enforced.** Slack/Telegram prior-flow retirement is still pending; a few non-blocking historical archival gaps remain as listed in §10 and §16.
