...
Version: 2026-04-26 v14
Status: populated with current-state evidence and re-aligned to package-v2 Decision 15 baseline; billing gate is confirmed resolved by self-hosted runner cutover (workflow run `24934831397` 13s success on 2026-04-25); plan-gate (private-repo branch protection / rulesets API) still returns plan-gated 403 and is now tracked separately from billing; Track A definitively confirms a Slack bot token (`xoxb-9849432992692-…`) is present in `똘똘이mk2` git history, escalating that token to required R1 rotation. Remaining blockers documented in §10 and §16
Patch record:
...
| GitHub branch-protection / rulesets enforcement proof | proposer-approver technical enforcement on `main` is still policy-only because branch-protection and rulesets APIs continue to return `403 Upgrade to GitHub Pro or make this repository public` for `hakhamsolution/ax-audit` (re-confirmed 2026-04-26). Billing-gate is no longer the cause: workflow run `24934831397` (workflow_dispatch on 2026-04-25) completed in 13 seconds on the `hakham-vps` self-hosted runner, proving CI execution is not blocked. | GitHub hosting plan / repository visibility / repository settings | high | choose one of: upgrade `hakhamsolution` to GitHub Pro, move governance repo to an organization tier that supports private-repo protection, or flip `ax-audit` to public if policy permits; capture settings proof during WB2 immediately afterward |
...
|---|---|---|---|---|---|
| promote automation | 강은구 | GitHub PR + written approval intended; CI execution itself is now unblocked (self-hosted runner verified 2026-04-25), but technical PR-only enforcement on `main` is still policy-only because branch-protection / rulesets API remains plan-gated 403 | partial | AI | 강은구 |
...
| change guard rule | 강은구 | intended PR + independent AI review; technical enforcement on `main` still blocked on private-repo plan-gate (branch-protection / rulesets API 403) | partial | AI | 강은구 |
...
3. Resolve the remaining GitHub hosting blocker so PR-only enforcement can be proven technically. The Actions billing gate is no longer active (self-hosted runner cutover verified 2026-04-25 with run `24934831397`); the only remaining gate is the private-repo branch-protection / rulesets plan gate, which still returns 403.
4. Slack/Telegram cutover decision is now actionable: Track A confirmed a plain-text Slack bot token of the form `xoxb-9849432992692-…` is present in `똘똘이mk2` git history, so the previously planned "reuse existing tokens" stance must change to "rotate the exposed Slack bot/app token (and as a precaution the Telegram bot token) before relying on them for AX cutover", treating both as R1 rotations under §15.3 (AI propose + 강은구 approve, no γ gate required).
...
As of 2026-04-26: the GitHub Actions billing gate is resolved by self-hosted runner cutover and is no longer counted as a blocker; the remaining hosting blocker is private-repo branch-protection / rulesets plan-gate; Track A is finalized with a confirmed Slack bot-token exposure in `똘똘이mk2` history, so R1 rotation is now the next required action (no γ gate); Slack/Telegram prior-flow retirement is still pending; and a few non-blocking historical archival gaps remain as listed in §10 and §16.
